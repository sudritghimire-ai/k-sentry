// v29: streamed two-beat exact sequence fast path with rare carry slow-path.
// Parser presents high 32 bits then low 32 bits on seq_hi_valid/seq_lo_valid.
// Normal accepted packets update only expected low16; any low16 carry goes slow-path.

module seq_stream29_bench #(
 ...
 always_ff @(posedge clk) begin
   if(rst) begin
     ...
     hi_bad<=0;lo_bad_q<=0;sess_q<=0;hb_q<=0;eos_q<=0;normal_q<=0;carry_q<=0;semok_q<=0;next_lo_q<=0;
     ...
     if(recover_valid) begin
       ...
       // Commit result from previous low beat.
       if(lo_bad_q || hi_bad || !sess_q) begin
         if(lo_bad_q || hi_bad || !sess_q) begin mismatch<=1; slow_required<=1; end
       end else if(eos_q) begin
         mismatch<=0;eos<=1;
       end else if(hb_q) begin
         mismatch<=0;accept_fast<=1;
