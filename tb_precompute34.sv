`timescale 1ns/1ps
module tb_precompute34;
 logic clk=0,rst=1,init_valid=0,seq_hi_valid=0,seq_lo_valid=0,count_valid=0,session_ok=1,valid_sem=0;
 logic[63:0]init_last_seq;logic[15:0]init_last_count,rx_count;logic[31:0]seq_hi,seq_lo;logic[135:0]semantic_bytes;
 logic accept_fast,slow_path,mismatch,status;
 hft_precompute_jordan34_bench dut(.*);
 always #1 clk=~clk;

 integer i,accepted=0,bad=0,special=0,carry_rec=0;
 logic[63:0] model_last,next_seq;logic[15:0] model_count;logic[16:0] sum;

 task initialize(input logic[63:0] s,input logic[15:0] c);
  begin
   @(negedge clk);init_last_seq=s;init_last_count=c;init_valid=1;
   @(negedge clk);init_valid=0;model_last=s;model_count=c;
  end
 endtask
 ...
 task packet(input logic[63:0] s,input logic[15:0] c,input logic sess,
  ...
  begin
   ...
   if(accept_fast!==ea || slow_path!==es || mismatch!==em) begin
    $display("FAIL i=%0d seq=%h cnt=%h got=%0d/%0d/%0d exp=%0d/%0d/%0d",
 ...
 initial begin
  init_last_seq=64'h1234_5678_0000_1000;init_last_count=20;seq_hi=0;seq_lo=0;rx_count=0;semantic_bytes=0;
  repeat(3)@(negedge clk);rst=0;initialize(init_last_seq,init_last_count);

  for(i=0;i<500000;i=i+1) begin
   next_seq=model_last+model_count;
   sum={1'b0,model_last[15:0]}+model_count;
   case(i%131)
    0: begin packet(next_seq+7,12,1,0,1,!sum[16]);bad=bad+1;end
    1: begin packet(model_last,12,1,0,1,!sum[16]);bad=bad+1;end
    2: begin packet(next_seq,12,0,0,1,0);bad=bad+1;end
    3: begin packet(next_seq,16'hffff,1,0,1,0);special=special+1;end
    4: begin
      if(!sum[16]) begin packet(next_seq,0,1,1,0,0);model_last=next_seq;model_count=0;accepted=accepted+1;end
      else begin packet(next_seq,0,1,0,1,0);initialize(next_seq,0);carry_rec=carry_rec+1;end
    end
    default: begin
      logic[15:0] nc;nc=(i%64)+1;
      if(!sum[16]) begin
        packet(next_seq,nc,1,1,0,0);model_last=next_seq;model_count=nc;accepted=accepted+1;
      end else begin
  ...
  $display("PASS: 500000 self-checked precomputed-expected MoldUDP64 decisions");
  $display("PASS: next packet decision uses equality only; carry diverts exactly");
